 ...
 */
declare class InsertSequence {
  constructor(generator: any);

  previousComplement(): Insert;

  current(max: number): None | number;

  next(): Insert;
}

declare type Shuffler = any;

declare type StructureLike = any;

declare type Shape = any;

declare interface Vertical {
  atVector: (vector: Vector) => number;
  atDimension: (image: HTMLImageElement | HTMLCanvasElement) => number;
}

declare interface Horizontal {
  atVector: (vector: Vector) => number;
  atDimension: (image: HTMLImageElement | HTMLCanvasElement) => number;
}

declare type Pair = [number, number, number, number];

declare type Tab = {
  isSlot: () => false;
  isTab: () => true;
  isNone: () => false;
  match: (other: any) => void;
  toString: () => "Tab";
  complement: () => Slot;
  serialize: () => "T";
};
 ...
 */
declare type CanvasConnectionListener = (
  piece: Piece,
  figure: Figure,
  targetPiece: Piece,
  targetFigure: Figure
 ...
 */
declare type CanvasTranslationListener = (
  piece: Piece,
  figure: Figure,
  dx: number,
  dy: number
...
declare class Squared {
  constructor(any);
  draw(piece: Piece, size: number, borderFill: number): number[]
  isBezier(): false
}
...
declare class Rounded {
   ...
   * @returns {number[]}
   */
  draw(p: Piece, size: number, borderFill: number): number[]
...
declare type TranslationListener = (
  piece: Piece,
  dx: number,
  dy: number
) => void;
...
declare class Piece {
   ...
   */
  export(options: { compact?: boolean }): PieceDump;
  /**
...
declare type Mapper<A, B> = (value: A) => B;

/**
 * Misc generic functions
 */
declare module "Prelude" {
  function pivot<T>(one: T, other: T, back?: boolean): any;
